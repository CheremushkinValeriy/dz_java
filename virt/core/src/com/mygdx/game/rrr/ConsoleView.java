...
public class ConsoleView {
        ...
        for (int i = 0; i < Main.GANG_SIZE; i++) {
            if (Main.whiteSide.get(i).getPosition().isEquals(position)) {
                if (Main.whiteSide.get(i).getHp() == 0)
                    str = "|" + AnsiColors.ANSI_RED + Main.whiteSide.get(i).getName().toUpperCase().charAt(0)
                            + AnsiColors.ANSI_RESET;
                else {
                    str = "|" + AnsiColors.ANSI_GREEN + Main.whiteSide.get(i).getName().toUpperCase().charAt(0)
                            + AnsiColors.ANSI_RESET;
                    alive = true;
                }
            }
            if (Main.darkSide.get(i).getPosition().isEquals(position) && !alive) {
                if (Main.darkSide.get(i).getHp() == 0)
                    str = "|" + AnsiColors.ANSI_RED + Main.darkSide.get(i).getName().toUpperCase().charAt(0)
                            + AnsiColors.ANSI_RESET;
                else
                    str = "|" + AnsiColors.ANSI_BLUE + Main.darkSide.get(i).getName().toUpperCase().charAt(0)
                            + AnsiColors.ANSI_RESET;
            }
        }
        return str;
    ...
    private static String PrintInfo(int npcIndex) {
        ...
        if (Main.whiteSide.get(npcIndex).getHp() == 0)
            ...
            str += "     " + AnsiColors.ANSI_GREEN + Main.whiteSide.get(npcIndex).getInfo() + AnsiColors.ANSI_RESET;
        if (Main.darkSide.get(npcIndex).getHp() == 0)
            str += "     " + AnsiColors.ANSI_RED + Main.darkSide.get(npcIndex).getInfo() + AnsiColors.ANSI_RESET;
        else
            str += "     " + AnsiColors.ANSI_BLUE + Main.darkSide.get(npcIndex).getInfo() + AnsiColors.ANSI_RESET;
